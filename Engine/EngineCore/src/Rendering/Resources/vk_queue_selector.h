...
typedef struct vqs__BinaryGraph
{
    ...
    uint32_t leftCount, rightCount,
        nodeCount; // leftCount = queueFamilyCount, rightCount = queueRequirementCount
...
static uint32_t vqs__u32_min(uint32_t a, uint32_t b)
{
    return a < b ? a : b;
}

static int32_t vqs__i32_min(int32_t a, int32_t b)
{
    return a < b ? a : b;
}

static float vqs__f32_min(float a, float b)
{
    return a < b ? a : b;
}

static float vqs__f32_max(float a, float b)
{
    return a > b ? a : b;
}
...
static void vqs__addEdge(vqs__Edge *edge, vqs__Edge *edgeRev, vqs__Node *from, vqs__Node *to,
                         int32_t cap, int32_t cost)
{
    edge->to = to;
    edge->next = from->head;
    edge->rev = edgeRev;
...
static int32_t vqs__graphDinicWithLimits(vqs__BinaryGraph *graph, int32_t flow,
                                         uint32_t *leftFlowLimits, uint32_t *leftFlowUsages)
{
    for (;;)
    {
        {
...
static uint32_t vqs__graphBuildInteriorEdges(vqs__BinaryGraph *graph, const VqsQuery query,
                                             bool buildEdge)
{
    uint32_t counter = 0;
        ...
        for (uint32_t i = 0; i < query->queueFamilyCount; ++i)
        {
                ...
                query->resultPresentQueueFamilyIndices[j] == UINT32_MAX)
            {
                ...
                if (query->vulkanFunctions.vkGetPhysicalDeviceSurfaceSupportKHR(
                        query->physicalDevice, i, requiredPresentQueueSurface, &presentSupport) !=
                    VK_SUCCESS)
                ...
                if (presentSupport && (flags & requiredFlags) == requiredFlags)
                {
                    if (buildEdge)
                    {
                        vqs__addEdge(graph->pInteriorEdges + counter,
                                     graph->pInteriorEdges + counter + 1, graph->pLeftNodes + i,
                                     graph->pRightNodes + j, 1,
...
static VkResult vqs__graphMainAlgorithm(vqs__BinaryGraph *graph, const VqsQuery query)
{
    ...
    if (flow < graph->rightCount)
    {
        for (uint32_t i = 0; i < graph->leftCount; ++i)
        {
            ...
            leftFlowLimits[i] =
                graph->rightCount > queueCount ? graph->rightCount - queueCount : 0u;
        }
        flow = vqs__graphDinicWithLimits(graph, flow, leftFlowLimits, leftFlowUsages);
...
static VkResult vqs__graphInit(vqs__BinaryGraph *graph, const VqsQuery query)
{
    ...
    for (uint32_t i = 0; i < graph->leftCount; ++i)
    { // S -> LEFT NODES
        vqs__addEdge(graph->pLeftEdges + (i * 2), graph->pLeftEdges + (i * 2 + 1), graph->pSNode,
                     graph->pLeftNodes + i, 0, 0);
    }
    for (uint32_t i = 0; i < graph->rightCount; ++i)
    { // RIGHT NODES -> T
        vqs__addEdge(graph->pRightEdges + (i * 2), graph->pRightEdges + (i * 2 + 1),
                     graph->pRightNodes + i, graph->pTNode, 1, 0);
    }
...
static VkResult vqs__queryFetchResults(VqsQuery query, const vqs__BinaryGraph *graph)
{
    ...
    for (uint32_t i = 0; i < graph->interiorEdgeCount; i += 2)
    {
        ...
        if (e->cap == 0)
        {
            query->resultQueueFamilyIndices[e->to - graph->pRightNodes] =
                e->rev->to - graph->pLeftNodes;
        }
    }
...
static VkResult vqs__queryInit(VqsQuery query, const VqsQueryCreateInfo *pCreateInfo)
{
    ...
    query->vulkanFunctions.vkGetPhysicalDeviceQueueFamilyProperties(
        query->physicalDevice, &query->queueFamilyCount,
    ...
    query->vulkanFunctions.vkGetPhysicalDeviceQueueFamilyProperties(
        query->physicalDevice, &query->queueFamilyCount, query->queueFamilyProperties);
...
void vqsGetQueueSelections(VqsQuery query, VqsQueueSelection *pQueueSelections)
{
    ...
    for (uint32_t i = 0; i < query->queueRequirementCount; ++i)
    {
        ...
        pQueueSelections[i].queueIndex = (--query->queueFamilyCounters[family]) %
        ...
        {
            pQueueSelections[i].presentQueueIndex = pQueueSelections[i].presentQueueFamilyIndex =
                UINT32_MAX;
        }
    }
}
                                        ...
                                        uint32_t *pQueuePriorityCount, float *pQueuePriorities)
{
    ...
    {
        uint32_t queueCount =
            vqs__u32_min(query->queueFamilyCounters[i], query->queueFamilyProperties[i].queueCount);
        if (pQueuePriorities)
    ...
    if (pQueuePriorities)
    {
        ...
        for (uint32_t i = 0; i < query->queueRequirementCount; ++i)
        {
            ...
            familyPriorityOffsets[family][queueIndex] = vqs__f32_max(
            ...
            if (family != UINT32_MAX)
            {
                ...
                familyPriorityOffsets[family][queueIndex] =
                    vqs__f32_max(familyPriorityOffsets[family][queueIndex],
