                                                       ...
                                                       Engine::Core::DependencyInjection::ServiceProvider *services)
{
    ...
    instance->m_Model =
        glm::rotate(glm::scale(glm::mat4(1.0f), glm::vec3(100, 100, 100)), glm::radians<float>(45), glm::vec3(0, 1, 0));

    // calculate MVP
    glm::mat4 mvp = instance->m_ProjectionMatrix * instance->m_View * instance->m_Model;

    // submit value to the renderer
    if (!services->GetRenderer()->QueueRender(&instance->m_Mesh->m_RendererCopy, &instance->m_Material->RendererID,
                                              mvp))
    {
